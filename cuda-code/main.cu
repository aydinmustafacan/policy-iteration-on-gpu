/**
 * @file main.cu
 * @brief Policy Iteration for discounted MDPs on the GPU using CSR sparsity.
 *
 * @details
 * Theory and algorithm (high-level):
 * - We solve a discounted Markov Decision Process (MDP) with finite state space S and action space A.
 *   For a fixed policy π, the value function V^π solves the Bellman equation:
 *     V^π = r^π + γ P^π V^π,
 *   where P^π ∈ R^{S×S} is the state-transition matrix induced by π and r^π ∈ R^S is the one-step reward.
 * - Policy Iteration alternates:
 *   1) Policy Evaluation: approximately solve V = r^π + γ P^π V (here via Jacobi fixed-point iteration)
 *   2) Policy Improvement: set π_{new}(s) = argmax_a Q(s,a), with
 *        Q(s,a) = ∑_{s'} P(s,a,s') [R(s,a,s') + γ V(s')]
 *   until the policy stabilizes.
 *
 * Data layout:
 * - We store the transition tensor P(s,a,s') as a single CSR matrix with (S⋅A) rows and S columns.
 *   Each row corresponds to a (state, action) pair. The reward values R(s,a,s') share the same sparsity
 *   pattern and ordering; if a separate CSR for rewards is provided, it is aligned row-by-row to P.
 *
 * GPU implementation highlights:
 * - Two evaluation paths exist:
 *   (A) A custom CUDA kernel (one block per state) that computes V_new[s] from the (s, π(s)) row.
 *   (B) A cuSPARSE-based path that constructs P^π (S×S CSR) and r^π, then iterates
 *       y = P^π V_old; V_new = r^π + γ y, using cusparseSpMV for the matrix-vector product.
 *   The current build uses the cuSPARSE path for better scalability.
 * - Policy improvement computes all Q(s, a) in parallel (one thread per action within a state block),
 *   then applies a deterministic tie-breaking rule: prefer the current action unless an alternative
 *   improves by more than a small epsilon (tie_eps).
 * - Convergence checks:
 *   • Inner evaluation loop stops when ||V_new − V_old||_∞ < θ (max-norm residual over states).
 *   • Outer loop stops when no policy entries change. We count changes on device to avoid H2D copies.
 * - Performance considerations:
 *   • Optional single precision (float) builds via USE_FP32 for higher throughput.
 *   • Avoid unnecessary global synchronizations; rely on D2H reads to serialize when needed.
 *   • "Ping-pong" pointer swapping for V and policy buffers avoids device-to-device copies each iteration.
 *   • cuSPARSE descriptors are reused across sweeps with updated pointers to the active V buffers.
 *
 * Limitations and assumptions:
 * - The simple improvement kernel assumes A ≤ 1024 (one thread per action per state block).
 * - Transition probabilities in each (s, a) row are normalized (optional normalization step provided).
 * - The inner evaluation uses Jacobi iteration (fixed-point); for very ill-conditioned problems, more
 *   advanced solvers (e.g., Gauss-Seidel, multigrid, Krylov with preconditioning) may be beneficial.
 *
 * Command-line usage:
 *   1) JSON (CSR):   ./cuda mdp.json
 *   2) CSV:          ./cuda transitions.csv rewards.csv
 *   3) Default demo: ./cuda
 *
 * JSON format (R supports two variants):
 * {
 *   "S": 9, "A": 4, "gamma": 0.9,
...
/**
 * @def USE_FP32
 * @brief When defined, compile the GPU math in single precision (float). Otherwise use double.
 */
#ifdef USE_FP32
using Real = float;
#else
...
// cuSPARSE for fast SpMV in evaluation
#include <cusparse.h>
...
/**
 * @brief cuSPARSE error checking helper.
 */
#define CUSPARSE_CHECK(err) if ((err) != CUSPARSE_STATUS_SUCCESS) { \
    std::cerr << "cuSPARSE Error at " << __FILE__ << ":" << __LINE__ << "\n"; \
    std::exit(1); \
}

// Map Real to cudaDataType for cuSPARSE compute type
/**
 * @brief Compile-time mapping from Real to cudaDataType for cuSPARSE.
 */
#if defined(USE_FP32)
static constexpr cudaDataType kCudaType = CUDA_R_32F;
#else
static constexpr cudaDataType kCudaType = CUDA_R_64F;
#endif
...
/**
 * @brief Split a comma-separated line into tokens, trimming whitespace.
 */
...
/**
 * @brief Consume the first line if it looks like a CSV header (contains @p indicator).
 */
...
/**
 * @brief Helper pair accumulating transition probability and reward for a (s,a,s') entry.
 */
struct PairPR { double p=0.0, r=0.0; };

/**
 * @brief Read MDP from two CSV files and build CSR over (S⋅A) rows with shared sparsity for P and R.
 * @param transition_file CSV with columns [s, a, s', p]
 * @param reward_file     CSV with columns [s, a, s', r]
 * @param[out] indptr,indices,P_data,R_data CSR arrays for (S⋅A)×S matrix
 * @param[out] S,A        number of states and actions
...
/**
 * @brief Read full file into memory.
 */
...
/**
 * @brief Extract a JSON numeric field by key from a simple object string.
 */
...
/**
 * @brief Get the raw content inside the first [...] that follows a key (very small JSON helper).
 */
...
template <typename T>
/**
 * @brief Parse a comma-separated list of numbers from a string into @p out.
 */
...
/**
 * @brief Read MDP from a JSON file, handling aligned R.data or separate R CSR (aligned to P).
 * @param filename JSON path
 * @param[out] indptr,indices,P_data,R_data CSR arrays for (S⋅A)×S matrix
 * @param[out] S,A,gamma problem dimensions and discount factor
 * @return true on success
 */
...
/**
 * @brief Evaluate a single Jacobi sweep for fixed policy using CSR rows of (s, π(s)).
 *
 * Each block computes one V_new[s]. Threads in the block iterate over the nnz of the row
 * (s, π(s)) to accumulate ∑_{s'} P(s,π(s),s') [R + γ V_old[s']].
 *
 * @note This kernel is kept for reference; the current implementation uses cuSPARSE SpMV.
 */
...
/**
 * @brief Policy improvement: compute Q(s,a) for all actions and choose greedy with tie-breaking.
 *
 * @param indptr,indices,P_data,R_data CSR storing rows for (s,a)
 * @param V_old Current value estimate
 * @param policy_curr Current policy (used for deterministic tie-breaking)
 * @param policy_new Output improved policy
 * @param S,A,gamma Problem sizes and discount
 *
 * @details Tie-breaking rule: prefer the current action unless another action improves Q by > tie_eps.
 */
...
/**
 * @brief Compute max-norm residual ||V_new − V_old||_∞ using a single-block reduction.
 */
...
// Count how many entries differ between two policy arrays on device
/**
 * @brief Count the number of indices where two device arrays differ (used for policy convergence).
 */
__global__ void count_policy_changes_kernel(
    ...
    int* __restrict__ out)
{
    ...
    for (int off = blockDim.x >> 1; off > 0; off >>= 1) {
        if (tid < off) smem_i[tid] += smem_i[tid + off];
        __syncthreads();
    }
    if (tid == 0) atomicAdd(out, smem_i[0]);
}

// Count nnz per policy-selected row
/**
 * @brief For each state s, compute nnz of the policy-selected row (s, π(s)).
 */
__global__ void count_pi_nnz(const int* indptr, const int* policy, int S, int A, int* row_nnz) {
    int s = blockIdx.x * blockDim.x + threadIdx.x;
    if (s >= S) return;
    int row = s * A + policy[s];
    row_nnz[s] = indptr[row + 1] - indptr[row];
}

// Build P_pi CSR (indices/data) and r_pi from (S*A)-row CSR and current policy
/**
 * @brief Build the CSR of P^π and the reward vector r^π from (S⋅A)-row CSR and current policy.
 *
 * Rows of P corresponding to (s, π(s)) are copied into contiguous rows of P^π. The reward vector
 * r^π[s] = ∑_{s'} P(s,π(s),s') R(s,π(s),s').
 */
__global__ void build_pi_csr_and_r(
    const int* __restrict__ indptr,
    const int* __restrict__ indices,
    const Real* __restrict__ P_data,
    const Real* __restrict__ R_data,
    const int* __restrict__ policy,
    const int* __restrict__ Ppi_indptr,
    int S, int A,
    int* __restrict__ Ppi_indices,
    Real* __restrict__ Ppi_data,
    Real* __restrict__ r_pi)
{
    int s = blockIdx.x * blockDim.x + threadIdx.x;
    if (s >= S) return;
    int row = s * A + policy[s];
    int beg = indptr[row];
    int end = indptr[row + 1];
    int out = Ppi_indptr[s];
    Real rsum = (Real)0;
    for (int j = beg; j < end; ++j, ++out) {
        Ppi_indices[out] = indices[j];
        Ppi_data[out]    = P_data[j];
        rsum += P_data[j] * R_data[j];
    }
    r_pi[s] = rsum;
}

// y = r_pi + gamma * y (in-place on y)
/**
 * @brief Compute y ← r^π + γ y over n elements (in-place axpy-like operation).
 */
__global__ void axpy_gamma_add(int n, Real gamma, const Real* __restrict__ r_pi, Real* __restrict__ y) {
    for (int i = blockIdx.x * blockDim.x + threadIdx.x; i < n; i += blockDim.x * gridDim.x) {
        y[i] = r_pi[i] + gamma * y[i];
    }
}

// ---------- Small util ----------
/**
 * @brief Get filename stem from a path (without extension).
 */
...
/**
 * @brief Get the parent directory name of a file path (last path component containing the file).
 */
...
/**
 * @brief Choose a base name for outputs. Prefer parent dir name for generic stems (e.g., "mdp").
 */
...
// ---------- MAIN ----------
/**
 * @brief Entry point. Loads an MDP, runs policy iteration on the GPU, and writes results/metrics.
 *
 * Program flow:
 * 1) Parse input (JSON or CSV) and construct CSR tensors (S⋅A)×S for P and aligned R.
 * 2) Allocate and upload data to device; configure cuSPARSE descriptors.
 * 3) Policy Iteration:
 *    - Build P^π (CSR) and r^π for current policy (device-side)
 *    - Policy evaluation via Jacobi: y = P^π V_old (cusparseSpMV), V_new = r^π + γ y,
 *      check residual; ping-pong swap V buffers between sweeps.
 *    - Policy improvement: compute Q(s, a) and choose greedy with deterministic tie-breaking.
 *    - Check convergence by counting policy changes on device; ping-pong swap policy buffers if needed.
 * 4) Copy back final policy and value; print and persist metrics.
 */
...
std::cout << "Precision: FP64 (double)\n";
    ...
    CUDA_CHECK(cudaMalloc(&d_changes, sizeof(int)));
    ...
    // cuSPARSE setup (SpMV-based evaluation)
    cusparseHandle_t sp_handle = nullptr;
    CUSPARSE_CHECK(cusparseCreate(&sp_handle));
    cusparseDnVecDescr_t vecX = nullptr, vecY = nullptr;
    CUSPARSE_CHECK(cusparseCreateDnVec(&vecX, S, d_V_old, kCudaType));
    CUSPARSE_CHECK(cusparseCreateDnVec(&vecY, S, d_V_new, kCudaType));
    cusparseSpMatDescr_t matPpi = nullptr; // created per-policy iteration
    Real alpha_one = (Real)1;
    Real beta_zero = (Real)0;
    // Buffers for policy-projected CSR and reward vector
    int*  d_Ppi_indptr = nullptr;  CUDA_CHECK(cudaMalloc(&d_Ppi_indptr, (size_t)(S + 1) * sizeof(int)));
    int*  d_Ppi_indices = nullptr; // allocated per-iteration based on nnz
    Real* d_Ppi_data    = nullptr; // allocated per-iteration based on nnz
    Real* d_r_pi        = nullptr; CUDA_CHECK(cudaMalloc(&d_r_pi, (size_t)S * sizeof(Real)));
    int*  d_row_nnz     = nullptr; CUDA_CHECK(cudaMalloc(&d_row_nnz, (size_t)S * sizeof(int)));
    ...
    // Warm-start: keep V_old from previous outer iteration (no zeroing)
        ...
        // Build P_pi CSR and r_pi for current policy
        int tpb = 256;
        int blks = (S + tpb - 1) / tpb;
        count_pi_nnz<<<blks, tpb>>>(d_indptr, d_policy, S, A, d_row_nnz);
        // exclusive scan into d_Ppi_indptr (d_Ppi_indptr[0]=0)
        CUDA_CHECK(cudaMemset(d_Ppi_indptr, 0, sizeof(int)));
        thrust::device_ptr<int> t_counts(d_row_nnz);
        thrust::device_ptr<int> t_indptr(d_Ppi_indptr);
        thrust::exclusive_scan(t_counts, t_counts + S, t_indptr + 1);
        int nnz_pi = 0;
        CUDA_CHECK(cudaMemcpy(&nnz_pi, d_Ppi_indptr + S, sizeof(int), cudaMemcpyDeviceToHost));
        if (d_Ppi_indices) CUDA_CHECK(cudaFree(d_Ppi_indices));
        if (d_Ppi_data)    CUDA_CHECK(cudaFree(d_Ppi_data));
        CUDA_CHECK(cudaMalloc(&d_Ppi_indices, (size_t)nnz_pi * sizeof(int)));
        CUDA_CHECK(cudaMalloc(&d_Ppi_data,    (size_t)nnz_pi * sizeof(Real)));
        build_pi_csr_and_r<<<blks, tpb>>>(
            d_indptr, d_indices, d_P_data, d_R_data, d_policy,
            d_Ppi_indptr, S, A, d_Ppi_indices, d_Ppi_data, d_r_pi);
        // create/update SpMat for P_pi
        if (matPpi) CUSPARSE_CHECK(cusparseDestroySpMat(matPpi));
        CUSPARSE_CHECK(cusparseCreateCsr(
            &matPpi, S, S, nnz_pi,
            d_Ppi_indptr, d_Ppi_indices, d_Ppi_data,
            CUSPARSE_INDEX_32I, CUSPARSE_INDEX_32I,
            CUSPARSE_INDEX_BASE_ZERO, kCudaType));
        // Allocate SpMV buffer for this policy
        size_t spmv_buf_bytes = 0; void* spmv_buf = nullptr;
        CUSPARSE_CHECK(cusparseSpMV_bufferSize(
            sp_handle, CUSPARSE_OPERATION_NON_TRANSPOSE,
            &alpha_one, matPpi, vecX, &beta_zero, vecY, kCudaType,
            CUSPARSE_SPMV_ALG_DEFAULT, &spmv_buf_bytes));
        CUDA_CHECK(cudaMalloc(&spmv_buf, spmv_buf_bytes));

        // Policy evaluation
        for (int ev = 0; ev < max_eval_iters; ++ev) {
            // y = P_pi * V_old -> d_V_new
            CUSPARSE_CHECK(cusparseSpMV(
                sp_handle, CUSPARSE_OPERATION_NON_TRANSPOSE,
                &alpha_one, matPpi, vecX, &beta_zero, vecY, kCudaType,
                CUSPARSE_SPMV_ALG_DEFAULT, spmv_buf));
            // V_new = r_pi + gamma * y
            axpy_gamma_add<<<blks, tpb>>>(S, gammaR, d_r_pi, d_V_new);
            ...
            // Swap V buffers (ping-pong) instead of copying the entire vector
            std::swap(d_V_old, d_V_new);
            // Update cuSPARSE dense-vector descriptors to the new pointers
            CUSPARSE_CHECK(cusparseDnVecSetValues(vecX, d_V_old)); // X = source (old)
            CUSPARSE_CHECK(cusparseDnVecSetValues(vecY, d_V_new)); // Y = dest (new)
            if ((double)delta_host < theta) break;
        }

        // Free SpMV scratch buffer
        if (spmv_buf) CUDA_CHECK(cudaFree(spmv_buf));
        ...
        // Convergence check on device: count changes between policies
        CUDA_CHECK(cudaMemset(d_changes, 0, sizeof(int)));
        int threads = 256;
        int blocks  = (S + threads - 1) / threads;
        if (blocks > 0) count_policy_changes_kernel<<<blocks, threads, threads * (int)sizeof(int)>>>(
            d_new_policy, d_policy, S, d_changes);
        int h_changes = 0;
    ...
    // Swap policy buffers (avoid D2D copy)
    std::swap(d_policy, d_new_policy);
        final_iterations = iter + 1;
    }
    ...
    if (sp_handle) CUSPARSE_CHECK(cusparseDestroy(sp_handle));
    cudaFree(d_indptr);
    cudaFree(d_indices);
    cudaFree(d_P_data);
    cudaFree(d_R_data);
